//! Shared XDR encoding, Stellar strkey, and base64 parsing utilities.
//!
//! This module is the single source of truth for the off-chain crates of the
//! stellar-router suite. Both `router-api-server` and `router-metrics-exporter`
//! historically maintained independent copies of these helpers; this module
//! consolidates that surface area so the crc16/strkey, base64, XDR writer,
//! transaction-envelope builder, and ScVal response parsing logic live in one
//! place.
//!
//! The implementation intentionally avoids the full `stellar-xdr` crate —
//! which requires the Rust `wasm32` toolchain target for Soroban SDK
//! dependencies — and hand-rolls exactly the subset needed off-chain:
//!
...
/// Minimal incremental XDR encoder for the Soroban constructs needed by the
/// off-chain crates.
pub struct XdrWriter(Vec<u8>);
...
/// Minimal incremental XDR decoder used when parsing ScVal responses.
...
/// Convenience wrapper around [`build_invoke_xdr`] used by consumers that
/// address the contract by its `C…` strkey and pass arguments as plain
/// strings (each encoded as `ScVal::String`).
pub fn build_invoke_xdr_from_strkey(
    contract_id: &str,
    function: &str,
    args: &[String],
) -> Result<String> {
    let contract_hash = decode_contract_id(contract_id)
        .map_err(|e| anyhow!("invalid contract id '{contract_id}': {e}"))?;
    let args: Vec<ScArg> = args.iter().map(|s| ScArg::String(s)).collect();
    Ok(build_invoke_xdr(&contract_hash, function, &args))
}
...
mod tests {
    ...
    #[test]
    fn encode_account_strkey_round_trip() {
        // Round-trip a known public key through the G… encoder.
        let key = [7u8; 32];
        let encoded = encode_account_strkey(&key);
        assert_eq!(encoded.len(), 56);
        assert!(encoded.starts_with('G'));
        let (version, decoded) = strkey_decode(&encoded).expect("decode");
        assert_eq!(version, VERSION_ACCOUNT);
    ...
    fn base64_invalid_input_fails() {
        // Length not congruent to 0 mod 4 (after padding trim) must error.
    ...
    #[test]
    fn build_invoke_xdr_from_strkey_invalid_id_fails() {
        let err = build_invoke_xdr_from_strkey("not-a-valid-contract-id", "fn", &[]);
        assert!(err.is_err());
    }
